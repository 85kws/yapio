...
export default function ManageBooking({ businessId, theme }) {
  ...
  const saveHours = async () => { await updateBusiness(businessId, { hours_json: hours }); Alert.alert('Kaydedildi', `Çalışma saatleri: ${hours.open}-${hours.close}`); };
  ...
  return (
    <ScrollView contentContainerStyle={s.wrap}>
      <Text style={s.h}>Çalışma Saatleri</Text>
      <View style={s.hoursRow}>
        <TimeBox label="Açılış" value={hours.open} onChange={(v) => setHours((h) => ({ ...h, open: v }))} />
        <TimeBox label="Kapanış" value={hours.close} onChange={(v) => setHours((h) => ({ ...h, close: v }))} />
        <TouchableOpacity style={[s.saveBtn, { backgroundColor: theme }]} onPress={saveHours}><Text style={s.saveText}>Kaydet</Text></TouchableOpacity>
      </View>

      <Text style={s.h}>Hizmetler</Text>
      {services.map((sv) => (
        <View key={sv.id} style={s.svc}>
          <View style={{ flex: 1 }}>
            <Text style={s.svcName}>{sv.data.name}</Text>
            <Text style={s.svcMeta}>{sv.data.duration_min} dk{sv.data.price ? ` · ${sv.data.price} ₺` : ''}</Text>
          </View>
      ...
      <View style={s.addBox}>
        <TextInput style={[s.input, { flex: 2 }]} value={name} onChangeText={setName} placeholder="Hizmet adı" placeholderTextColor="#B0B0C0" />
        <TextInput style={[s.input, { flex: 1 }]} value={dur} onChangeText={setDur} placeholder="dk" keyboardType="number-pad" placeholderTextColor="#B0B0C0" />
      ...
      <Text style={s.h}>Randevular ({pending.length})</Text>
      {pending.length === 0 && <Text style={s.empty}>Henüz randevu yok.</Text>}
      {pending.map((a) => (
        <View key={a.id} style={s.appt}>
          <View style={{ flex: 1 }}>
            <Text style={s.apptName}>{a.data.service_name} · {a.user_name || 'Müşteri'}</Text>
            <Text style={s.apptMeta}>{a.data.date} · {a.data.time}{a.status === 'confirmed' ? ' · ✓ onaylı' : ''}</Text>
          </View>
          {a.status !== 'confirmed' && (
            <TouchableOpacity style={[s.miniBtn, { backgroundColor: theme }]} onPress={() => setStatus(a.id, 'confirmed')}><Text style={s.miniText}>Onayla</Text></TouchableOpacity>
          )}
          <TouchableOpacity style={s.miniGhost} onPress={() => setStatus(a.id, 'cancelled')}><Text style={s.miniGhostText}>İptal</Text></TouchableOpacity>
        </View>
      ))}
