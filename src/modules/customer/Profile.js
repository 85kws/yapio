    ...
    <ScrollView contentContainerStyle={s.wrap}>
      ...
      {b.address ? (
        <TouchableOpacity style={s.row} onPress={() => Linking.openURL(`https://maps.google.com/?q=${encodeURIComponent(b.address)}`)}>
          ...
          <View style={{ flex: 1 }}><Text style={s.label}>Adres</Text><Text style={s.val}>{b.address}</Text></View>
      ...
      {b.phone ? (
        <TouchableOpacity style={s.row} onPress={() => Linking.openURL(`tel:${b.phone}`)}>
          ...
          <View style={{ flex: 1 }}><Text style={s.label}>Telefon</Text><Text style={s.val}>{b.phone}</Text></View>
      ...
      {(h.open || h.close) ? (
        <View style={s.row}>
          ...
          <View style={{ flex: 1 }}><Text style={s.label}>Çalışma Saatleri</Text><Text style={s.val}>{h.open || '—'} - {h.close || '—'}</Text></View>
        </View>
      ) : null}
