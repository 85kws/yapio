...
export default function Subscriptions({ businessId, theme }) {
  ...
  const subscribe = async (plan) => {
    ...
    Alert.alert('Üyelik başladı', plan.data.name); load();
  };
    ...
    <ScrollView contentContainerStyle={s.wrap}>
      {mine ? (
        <View style={[s.card, { backgroundColor: theme }]}>
          <View style={s.cardTop}><Ionicons name="ticket" size={26} color="#fff" /><Text style={s.cardLabel}>Aktif Üyelik</Text></View>
          <Text style={s.cardPlan}>{mine.data.plan_name}</Text>
          <Text style={s.cardMeta}>Başlangıç: {mine.data.started}  ·  Giriş: {mine.data.checkins || 0}</Text>
          <View style={s.qr}><Ionicons name="qr-code" size={90} color={theme} /><Text style={s.qrCode}>#{mine.id}</Text></View>
          <Text style={s.qrNote}>Girişte bu kodu personele göster.</Text>
        </View>
      ) : (
        <>
          <Text style={s.h}>Üyelik Planları</Text>
          {plans.length === 0 && <Text style={s.empty}>Henüz plan eklenmemiş.</Text>}
          {plans.map((p) => (
            <View key={p.id} style={s.plan}>
              ...
              <TouchableOpacity style={[s.subBtn, { backgroundColor: theme }]} onPress={() => subscribe(p)}><Text style={s.subText}>Abone Ol</Text></TouchableOpacity>
            </View>
          ))}
        </>
      )}
